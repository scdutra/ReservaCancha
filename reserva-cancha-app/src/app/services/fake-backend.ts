    ...
    backend.connections.subscribe((connection: MockConnection) => {
        ...
        setTimeout(() => {
            ...
            if (connection.request.url.endsWith('/login') && connection.request.method === RequestMethod.Post) {
                ...
                if (params.email === testUser.email && params.clave === testUser.clave) {
                    connection.mockRespond(new Response(
                        new ResponseOptions({ status: 200, body: { "persona": testUser, "token": "fake-jwt-token" } })
                    ));
                } else {
                    connection.mockRespond(new Response(
